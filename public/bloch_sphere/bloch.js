...
var loader, scene, camera, renderer, controls, ambiLight, sphere, xAxis, yAxis, zAxis, ket0, ket1, states;
...
function init_bloch_sphere(containerName='Bloch Sphere') {
  ...
  loader.load("./assets/droid_serif_regular.typeface.json",
    function (font) {
      var ket0Geometry = new TextGeometry('|0>', {
        font: font,
        size: 0.1,
        height: 0.01 });
      var ket1Geometry = new TextGeometry('|1>', {
        font: font,
        size: 0.1,
        height: 0.01 });
      var ketMaterial = new THREE.MeshPhongMaterial({color: 0xFFFFFF});
      ket0 = new THREE.Object3D();
      ket0.add(new THREE.Mesh(ket0Geometry, ketMaterial));
      ket0.position.set(0, 1.1, 0);
      scene.add(ket0);
      ket1 = new THREE.Object3D();
      ket1.add(new THREE.Mesh(ket1Geometry, ketMaterial));
...
function genLine(position, lineColor, name) {
  ...
  loader.load("assets/droid_serif_regular.typeface.json",
    function (font) {
      var nameGeometry = new TextGeometry(name, {
        font: font,
        size: 0.1,
        height: 0.01 });
